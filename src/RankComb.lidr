...
>   let lc = l .&&. 3
>       a1  = if lc /= 0 then acc .||. (lc .<<<. (spot - 1)) else acc
>       rc = r .&&. 3
>       a2 = if rc /= 0 then a1 .||. (rc .<<<. (spot + 1)) else a1
...
>                 in if ln < 4
>                   then App (unrankIKSC (assert_smaller n rn)) (unrankIKSC (assert_smaller n ln))
>                   else App (unrankIKSC (assert_smaller n ln)) (unrankIKSC (assert_smaller n rn))
...
> rankIKSC a@(App l r)    =
>   let l' = rankIKSC (assert_smaller a l)
>       r' = rankIKSC (assert_smaller a r)
>   in case r of
>       (PrimComb _ _) => combnum4 r' l'
>       _              => combnum4 l' r'
